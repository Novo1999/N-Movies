...
function Filter() {
  ...
  return (
    <div className="relative">
      <button
        className="w-6 lg:w-8  text-white md:text-xs lg:text-lg"
        onClick={() => setFilterIsOpened((open) => !open)}
      >
        <svg
          className="md:w-6 lg:w-10"
        ...
        <span className="relative text-xs lg:text-xl lg:right-0 md:right-1">
          Filter
        </span>
      </button>
      {/* Filter options */}
      {filterIsOpened && (
        <div className="absolute  w-96 h-100 text-white filter-bg z-10 left-[-6rem] lg:left-0 rounded-md p-10">
          <hr className="mb-4" />
          <div className="flex">
